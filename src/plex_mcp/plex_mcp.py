...
MAX_COUNT: int = 50
MAX_KEYS: int = 100
MAX_STATS_LEAVES: int = (
    5_000  # Max leaf items (episodes/tracks) scanned per section in get_library_stats
)
...
class PlexClient:
    ...
    def __init__(
        self, server_url: Optional[str] = None, token: Optional[str] = None
    ) -> None:
    ...
    def get_server(self) -> PlexServer:
        ...
        if self._server is not None:
            try:
                ...
                raise PlexMCPConnectionError(
                    "Unauthorized: Invalid Plex token provided."
                )
            except Exception:
                # Any other error (network blip, server restart, etc.) — reconnect below.
                logger.warning(
                    "Plex server health check failed; attempting to reconnect."
                )
                self._server = None
        ...
        try:
            ...
            raise PlexMCPConnectionError(
                f"Error initializing Plex server: {exc}"
            ) from exc
        return self._server
...
_plex_client_instance: Optional[PlexClient] = None
...
async def search_movies(
    ...
    try:
        ...
        try:
            movie_section = await asyncio.to_thread(
                lambda: plex.library.section("Movies")
            )
        except Exception:
            # Fall back to the first movie library if the name differs
            sections = await asyncio.to_thread(lambda: plex.library.sections())
            movie_section = next(
                (s for s in sections if getattr(s, "type", None) == "movie"), None
            )
        ...
        movies = await asyncio.to_thread(
            lambda: movie_section.search(maxresults=limit + 1, **filters)
        )
    ...
    logger.info(
        "Found %d movies (limit=%d) matching filters: %r", len(movies), limit, filters
    )
...
async def get_playlist_items(playlist_key: str) -> str:
    ...
    try:
        ...
        try:
            ...
            if not playlist:
                raise PlexMCPNotFoundError(
                    f"No playlist found with key {playlist_key}."
                )
        ...
        for i, item in enumerate(items, 1):
            ...
            if item_type == "track":
                ...
                formatted_items.append(
                    f"{i}. {item.title}  —  {artist} / {album}  ({dur_str})  [key: {item.ratingKey}]"
                )
            elif item_type == "episode":
                show = getattr(item, "grandparentTitle", "")
                season = getattr(item, "parentTitle", "")
                formatted_items.append(
                    f"{i}. {show} — {season}: {item.title}  [key: {item.ratingKey}]"
                )
            else:
                year = getattr(item, "year", "")
                year_str = f" ({year})" if year else ""
                formatted_items.append(
                    f"{i}. {item.title}{year_str}  [key: {item.ratingKey}]"
                )
...
async def create_playlist(name: str, item_keys: str) -> str:
    ...
    try:
        ...
        if not_found_keys:
            raise PlexMCPNotFoundError(
                f"Some item keys were not found: {', '.join(str(k) for k in not_found_keys)}"
            )
        if not items:
        ...
        raise PlexMCPError(
            f"Invalid input format. Please check item keys are valid numbers. {str(e)}"
        ) from e
    except Exception as e:
...
async def add_to_playlist(playlist_key: str, item_key: str) -> str:
    ...
    try:
        ...
        if (
            playlist_type
            and item_list_type
            and type_map.get(item_list_type) != playlist_type
        ):
            raise PlexMCPError(
        ...
        await asyncio.to_thread(lambda p=playlist, m=item: p.addItems([m]))  # type: ignore[misc]
        ...
        raise PlexMCPError(
            "Invalid playlist or item key. Please provide valid numbers."
        )
    except Exception as e:
...
async def recent_movies(count: int = DEFAULT_LIMIT) -> str:
    ...
    try:
        ...
        recent_movies_list = await asyncio.to_thread(
            lambda: movie_section.recentlyAdded(maxresults=count)
        )
        ...
        for i, movie in enumerate(recent_movies_list, 1):
            added_str = (
                movie.addedAt.strftime("%Y-%m-%d")
                if getattr(movie, "addedAt", None)
                else "Unknown"
            )
...
async def get_movie_genres(movie_key: str) -> str:
    ...
    try:
        ...
        raise PlexMCPError(
            f"Invalid movie key '{movie_key}'. Please provide a valid number."
        )
    except Exception as e:
...
async def most_watched(
    ...
    try:
        ...
        all_items = await asyncio.to_thread(
            lambda: section.search(sort=sort, maxresults=count)
        )
        ...
        for i, item in enumerate(all_items, 1):
            ...
            if media_type == "movies":
                ...
                detail = (
                    f"last watched {last_viewed.strftime('%Y-%m-%d')}"
                    if last_viewed
                    else "watched"
                )
            results.append(
                f"{i}. {item.title} ({year}) — {detail}  [key: {item.ratingKey}]"
            )
...
async def get_watch_history(count: int = DEFAULT_COUNT) -> str:
    ...
    try:
        ...
        for i, item in enumerate(history, 1):
            ...
            viewed_str = (
                viewed_at.strftime("%Y-%m-%d %H:%M") if viewed_at else "Unknown"
            )
...
async def get_on_deck() -> str:
    ...
    try:
        ...
        for i, item in enumerate(on_deck, 1):
            ...
            results.append(
                f"{i}. [{item_type}] {display}  ({progress})  [key: {item.ratingKey}]"
            )
...
async def get_library_stats() -> str:
    ...
    try:
        ...
        for section in sections:
            items = await asyncio.to_thread(lambda s=section: s.all())  # type: ignore[misc]
            count = len(items)
            ...
            total_size = sum(
                sum(
                    getattr(part, "size", 0) or 0
                    for media in getattr(leaf, "media", [])
            ...
            capped = len(leaves) == MAX_STATS_LEAVES and section.type in (
                "show",
                "artist",
            )
...
async def search_tv_shows(
    ...
    try:
        ...
        shows = await asyncio.to_thread(
            lambda: section.search(maxresults=limit + 1, **filters)
        )
        if not shows:
...
async def get_show_details(show_key: str) -> str:
    ...
    try:
        ...
        for season in seasons:
            ...
            lines.append(
                f"  {season.title}: {ep_count} episodes  [key: {season.ratingKey}]"
            )
...
async def search_music(
    ...
    if not any([artist, album, track, genre]):
        raise PlexMCPError(
            "Provide at least one search parameter (artist, album, track, or genre)."
        )
    ...
    try:
        ...
        if track:
            ...
            items = await asyncio.to_thread(
                lambda: section.searchTracks(maxresults=limit, **filters)
            )
            for i, t in enumerate(items, 1):
                ...
                results.append(
                    f"{i}. {t.title}  —  {artist_name} / {album_title}  ({duration // 60}:{duration % 60:02d})  [key: {t.ratingKey}]"
                )
            ...
            if artist:
                filters["artist.title"] = artist
            items = await asyncio.to_thread(
                lambda: section.searchAlbums(maxresults=limit, **filters)
            )
            for i, a in enumerate(items, 1):
                ...
                results.append(
                    f"{i}. {a.title} ({year})  —  {artist_name}  ({track_count} tracks)  [key: {a.ratingKey}]"
                )
            ...
            if genre:
                filters["genre"] = genre
            items = await asyncio.to_thread(
                lambda: section.searchArtists(maxresults=limit, **filters)
            )
            for i, a in enumerate(items, 1):
                genres = ", ".join(g.tag for g in getattr(a, "genres", [])[:3])
                album_count = getattr(a, "childCount", "?")
                results.append(
                    f"{i}. {a.title}  —  {genres or 'Unknown genre'}  ({album_count} albums)  [key: {a.ratingKey}]"
                )
...
async def get_artist_details(artist_key: str) -> str:
    ...
    try:
        ...
        for album in albums:
            ...
            lines.append(
                f"  {album.title} ({year})  —  {track_count} tracks  [key: {album.ratingKey}]"
            )
...
async def get_album_details(album_key: str) -> str:
    ...
    try:
        ...
        for track in tracks:
            ...
            lines.append(
                f"  {track_num}. {track.title}  ({duration_s // 60}:{duration_s % 60:02d})  [key: {track.ratingKey}]"
            )
